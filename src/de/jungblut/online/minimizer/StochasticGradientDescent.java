...
public class StochasticGradientDescent implements StochasticMinimizer {
	...
	private static final Log LOG = LogFactory.getLog(StochasticGradientDescent.class);
	...
	public static class StochasticGradientDescentBuilder {
		 ...
		 * 
		 * @param momentum
		 *            the momentum to use. Between 0 and 1.
		 * @return the builder again.
		 */
		public StochasticGradientDescentBuilder momentum(double momentum) {
			Preconditions.checkArgument(momentum >= 0d && momentum <= 1d, "Momentum must be between 0 and 1.");
			this.momentum = momentum;
			return this;
		}

		/**
		 * Holdout validation percentage, this will take a subset of the data on
		 * the stream and do a validation on it.
		 * 
		 * @param perc
		 *            the percentage to use. Between 0 and 1.
		 * @return the builder again.
		 */
		public StochasticGradientDescentBuilder holdoutValidationPercentage(double perc) {
			Preconditions
					.checkArgument(momentum >= 0d && momentum <= 1d, "HoldOut Percentage must be between 0 and 1.");
			this.holdoutValidationPercentage = perc;
			return this;
		}

		/**
		 * Sets the weight updater, for example to use regularization. The
		 * default is the normal gradient descent.
		 * 
		 ...
		 * @return the builder again.
		 */
		public StochasticGradientDescentBuilder weightUpdater(WeightUpdater weightUpdater) {
		...
		/**
		 * Sets the size of the history to keep to compute average improvements
		...
		/**
		 * Sets the progress report interval. Since writing to the console/log
		 * might be expensive, this is an easy way to limit the logging if
		 * needed.
		 * 
		 * @param interval
		 *            the interval. E.g. every 10th iteration.
		 * @return the builder again.
		 */
		 ...
		 * 
		 * @param delta
		 *            the delta to break in difference between two costs.
		 * @return the builder again.
		 */
		...
		/**
		 * Sets a simple annealing (alpha / (1+current_iteration / phi)) where
		 * phi is the given parameter here. This will gradually lower the global
		 * learning rate after the given amount of iterations.
		 * 
		 * @param iteration
		 *            the iteration to start annealing.
		 * @return the builder again.
		 */
		public StochasticGradientDescentBuilder annealingAfter(int iteration) {
			Preconditions.checkArgument(iteration > 0, "Annealing can only kick in after the first iteration! Given: "
					+ iteration);
	...
	@Override
	public DoubleVector minimize(DoubleVector start, Supplier<Stream<FeatureOutcomePair>> streamSupplier,
		...
		for (int pass = 0; pass < numPasses; pass++) {
			...
			currentStream.forEach((next) -> doStep(passFinal, next, costFunction, verbose));
	...
	private void doStep(int pass, FeatureOutcomePair next, StochasticCostFunction costFunction, boolean verbose) {
		...
		try {
			...
			if (verbose) {
				...
				if (iteration % progressReportInterval == 0) {
					LOG.info("Pass " + pass + " | Iteration " + iteration + " | Validation Cost: " + validationError
							/ Math.max(validationItems, 1) + " | Last Cost: " + observed.getCost()
		...
		try {
			...
			CostWeightTuple update = weightUpdater.computeNewWeights(theta, observed.getGradient(), alpha, iteration,
					lambda, observed.getCost());
